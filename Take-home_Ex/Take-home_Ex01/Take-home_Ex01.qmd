...
```{r}
roads_sf <- st_set_geometry(roads_sf, "LINESTRING")

```

```{r}
lixels <- lixelize_lines(roads_sf, 750, mindist = 375)
```

## East Region

```{r}
roads_in_east <- st_as_sf(roads_in_east, wkt = "geometry")
lixels <- spNetwork::lixelize_lines(st_coordinates(roads_in_east),
                                     750, 
                                     mindist = 375)
```
